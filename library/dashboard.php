...
$params = [];
if($filterCat > 0)     { $bookSql .= " AND b.CatId = :cat";                                          $params[':cat']    = $filterCat; }
if($filterAuthor > 0)  { $bookSql .= " AND b.AuthorId = :author";                                    $params[':author'] = $filterAuthor; }
if($searchQuery != '')  { $bookSql .= " AND (b.BookName LIKE :search OR a.AuthorName LIKE :search)";  $params[':search'] = '%'.$searchQuery.'%'; }
$bookSql .= " ORDER BY b.RegDate DESC";

$bookQuery = $dbh->prepare($bookSql);
foreach($params as $k => $v) $bookQuery->bindValue($k, $v, PDO::PARAM_STR);
$bookQuery->execute();
$books = $bookQuery->fetchAll(PDO::FETCH_OBJ);

$activeCatName = '';
if($filterCat > 0) foreach($categories as $c) { if($c->id == $filterCat) { $activeCatName = $c->CategoryName; break; } }
$activeAuthorName = '';
if($filterAuthor > 0) foreach($authors as $a) { if($a->id == $filterAuthor) { $activeAuthorName = $a->AuthorName; break; } }
?>
<!DOCTYPE html>
...
<head>
    ...
    <!-- ✅ Shared header: injects :root variables + nav CSS + nav HTML -->
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .filter-option {
            ...
            padding: 5px 6px; border-radius: 6px; cursor: pointer; transition: background 0.15s;
        }
        .filter-option:hover { background: var(--bg); }
        .filter-option input[type="checkbox"] {
            width: 15px; height: 15px; accent-color: var(--blue); cursor: pointer; flex-shrink: 0;
        }
        .filter-option label { font-size: 13px; color: var(--text); cursor: pointer; line-height: 1.3; }
        .btn-apply-filter {
        ...
        .btn-clear-filter {
            ...
            font-size: 12px; cursor: pointer; margin-top: 6px; text-align: center;
        ...
        /* ── MAIN ─────────────────────────────────── */
        .main-content { flex: 1; padding: 24px 24px 40px; min-width: 0; }
        ...
        .stat-card {
            ...
            gap: 14px; text-decoration: none; transition: box-shadow 0.2s, transform 0.2s;
        }
        .stat-card:hover { box-shadow: 0 4px 16px rgba(0,0,255,0.08); transform: translateY(-2px); text-decoration: none; }
        .stat-icon {
        ...
        .section-header h2 { font-family: 'Playfair Display', serif; font-size: 20px; color: var(--text); }
        .result-count {
            font-size: 12px; color: var(--text-muted); background: var(--white);
        ...
        /* ── BOOK GRID ────────────────────────────── */
        .book-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(148px, 1fr)); gap: 16px; }
        .book-card {
            background: var(--white); border-radius: 10px; overflow: hidden;
            border: 1px solid var(--border); transition: box-shadow 0.2s, transform 0.2s;
            text-decoration: none; display: block;
        }
        .book-card:hover { box-shadow: 0 6px 20px rgba(0,0,255,0.10); transform: translateY(-3px); text-decoration: none; }
        .book-cover-wrap {
            width: 100%; aspect-ratio: 2/3; overflow: hidden; position: relative;
            background: linear-gradient(135deg, var(--blue-soft) 0%, #d0d0f8 100%);
        }
        .book-cover-wrap img { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform 0.3s; }
        ...
        .book-cover-placeholder span { font-size: 11px; font-weight: 600; color: var(--blue); text-align: center; line-height: 1.3; opacity: 0.6; }
        .book-info { padding: 10px 10px 12px; }
        .book-title {
            font-size: 12px; font-weight: 600; color: var(--text); margin-bottom: 3px; line-height: 1.35;
            display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;
        }
        .book-author { font-size: 11px; color: var(--text-muted); margin-bottom: 7px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .badge-row { display: flex; flex-wrap: wrap; gap: 4px; }
        .book-badge { display: inline-block; font-size: 9px; font-weight: 700; letter-spacing: 0.4px; padding: 2px 7px; border-radius: 20px; text-transform: uppercase; }
        .badge-available { background: #e8f5e9; color: #2e7d32; }
        .badge-reserved  { background: #fff3e0; color: #e65100; }
        .badge-cat       { background: var(--blue-soft); color: var(--blue); }
        .no-books { grid-column: 1 / -1; text-align: center; padding: 70px 20px; color: var(--text-muted); }
        ...
        <form method="get" action="dashboard.php" id="filterForm">
            ...
            <div class="filter-section">
                ...
                <div class="filter-option">
                    <input type="checkbox" name="cat" id="cat_<?php echo $cat->id; ?>"
            ...
            <div class="filter-section">
                ...
                <div class="filter-option">
                    <input type="checkbox" name="author" id="auth_<?php echo $auth->id; ?>"
    ...
    <main class="main-content">
        ...
        <div class="active-filters">
            ...
            <span class="filter-pill">
                <i class="fa fa-tag" style="font-size:10px;"></i> <?php echo htmlentities($activeCatName); ?>
                <a href="dashboard.php?<?php echo $filterAuthor > 0 ? 'author='.$filterAuthor.'&' : ''; echo $searchQuery != '' ? 'search='.urlencode($searchQuery) : ''; ?>">&times;</a>
            </span>
            <?php endif; ?>
            <?php if($filterAuthor > 0): ?>
            <span class="filter-pill">
                <i class="fa fa-user" style="font-size:10px;"></i> <?php echo htmlentities(trim($activeAuthorName)); ?>
                <a href="dashboard.php?<?php echo $filterCat > 0 ? 'cat='.$filterCat.'&' : ''; echo $searchQuery != '' ? 'search='.urlencode($searchQuery) : ''; ?>">&times;</a>
            </span>
            <?php endif; ?>
            <?php if($searchQuery != ''): ?>
            <span class="filter-pill">
                <i class="fa fa-search" style="font-size:10px;"></i> &ldquo;<?php echo htmlentities($searchQuery); ?>&rdquo;
                <a href="dashboard.php?<?php echo $filterCat > 0 ? 'cat='.$filterCat.'&' : ''; echo $filterAuthor > 0 ? 'author='.$filterAuthor : ''; ?>">&times;</a>
            </span>
            <?php endif; ?>
        </div>
        <?php endif; ?>
